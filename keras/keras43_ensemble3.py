...
model = Model(inputs=[input1, input2,input3], outputs=[last_output, last_output3])
...
model.fit([x1_train, x2_train,x3_train], [y1_train,y2_train], epochs=1005, batch_size=32,     # 두개 이상은 list이므로, list형식으로 해준다.
...
loss = model.evaluate([x1_test,x2_test,x3_test],[y1_test,y1_test])
result1, result2 = model.predict([x1_test,x2_test,x3_test]) 
